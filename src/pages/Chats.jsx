...
const Chats = () => {
  ...
  return (
    <div className='w-full gap-8 flex h-screen mt-20 md:mt-22    overflow-hidden '>
      ...
      <button onClick={() => setOpen(!open)} data-drawer-target="logo-sidebar" data-drawer-toggle="logo-sidebar" aria-controls="logo-sidebar" type="button" className="flex items-start p-2 mt-[-6px] text-sm text-yellow-400 rounded-lg sm:hidden focus:outline-none">

  <svg class="w-7 h-7" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 18">
   ...
   <span onClick={() => setOpen(!open)} className='absolute top-1 font-semibold right-2'>
    ...
    {chatsArr.length > 0 ? (
          chatsArr.map((user, index) => (
            ...
            <li key={index} className='flex justify-start gap-1' onClick={() => {handleSelectedFriend(user); setOpen(!open)} }>
            <img
            src={user?.profilePic || 'https://via.placeholder.com/50'}
            alt={`${user?.name}'s profile`}
